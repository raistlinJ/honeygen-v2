...
EXEC_PATTERN = re.compile(
    rf"Executed instruction at:\s*{HEX_CAPTURE}(?:\s*\[[^\]]+\])?\s*-\s*(?P<instruction>.+)"
)
...
def iter_log_entries(log_input: str | Path) -> Iterator[dict[str, str]]:
    for raw_line in _line_source(log_input):
        ...
        yield {"address": address, "instruction": instruction}


def parse_log(log_input: str | Path) -> list[dict[str, str]]:
    return list(iter_log_entries(log_input))
